...
db.serialize(() => {
    ...
    // Otomatik içerik kodları burada (yer kaplamasın diye kısalttım, öncekiyle aynı)
    db.get("SELECT COUNT(*) as count FROM blog", (err, row) => {
        if (row && row.count === 0) {
            const dummyAgents = ['XANTARES_Fan', 's1mple_clone', 'DonkPeek', 'RushB_PLZ'];
            const topics = [{t: "CS2 Optimizasyon", c: "FPS sorunu var mı?", cat: "Bug"}];
            topics.forEach(t => {
                const randomAuthor = dummyAgents[Math.floor(Math.random() * dummyAgents.length)];
                db.run("INSERT INTO blog (user_id, title, author, content, category) VALUES (0, ?, ?, ?, ?)", [t.t, randomAuthor, t.c, t.cat]);
            });
        }
    });
});
...
app.use(session({ secret: 'komutan_logar_v22', resave: false, saveUninitialized: true }));
    ...
    db.get('SELECT COUNT(*) as uCount FROM users', (err, uRow) => {
        db.get('SELECT COUNT(*) as pCount FROM blog', (err, pRow) => {
            db.all('SELECT * FROM blog ORDER BY created_at DESC LIMIT 6', (err, posts) => {
                res.render('index', { recentPosts: posts || [], stats: { users: uRow ? uRow.uCount : 0, posts: pRow ? pRow.pCount : 0, online: onlineCount } });
            });
        });
    });
});
...
app.get('/community', (req, res) => {
    ...
    db.all(sql, params, (err, posts) => {
        db.all('SELECT post_id FROM comments', (err, allComments) => {
             if(posts) { posts.forEach(p => { p.commentCount = allComments.filter(c => c.post_id === p.id).length; if(p.user_id === 0) p.authorAvatar = '/img/default_avatar.png'; }); }
...
app.post('/community/new', isAuthenticated, (req, res) => {
    db.run(`INSERT INTO blog (user_id, title, author, content, category) VALUES (?, ?, ?, ?, ?)`, [req.session.user.id, req.body.title, req.session.user.username, req.body.content, req.body.category], () => res.redirect('/community'));
});
app.post('/community/delete/:id', isAuthenticated, (req, res) => {
    db.get('SELECT * FROM blog WHERE id = ?', [req.params.id], (err, post) => {
        if(post && post.user_id === req.session.user.id) { db.run('DELETE FROM blog WHERE id = ?', [req.params.id], () => res.redirect('/community')); } else { res.redirect('/community'); }
    });
});
app.get('/post/:id', (req, res) => {
    db.get(`SELECT blog.*, users.username as authorName, users.avatar as authorAvatar FROM blog LEFT JOIN users ON blog.user_id = users.id WHERE blog.id = ?`, [req.params.id], (err, post) => {
        ...
        db.all(`SELECT comments.*, users.avatar as userAvatar FROM comments JOIN users ON comments.user_id = users.id WHERE post_id = ? ORDER BY created_at ASC`, [req.params.id], (err, comments) => res.render('post_detail', { post, comments: comments || [] }));
    });
});
...
app.post('/comment/delete/:id', isAuthenticated, (req, res) => {
    db.get('SELECT * FROM comments WHERE id = ?', [req.params.id], (err, comment) => {
        if(comment && comment.user_id === req.session.user.id) { db.run('DELETE FROM comments WHERE id = ?', [req.params.id], () => res.redirect('/post/' + comment.post_id)); } else { res.redirect('/community'); }
    });
});

// --- ARKADAŞLAR (GÜNCELLENDİ: TİM ÜYELERİNİ ÇEKİYOR) ---
        ...
        db.all(`SELECT u.username, u.id as user_id, f.id as request_id FROM friends f JOIN users u ON f.sender_id = u.id WHERE f.receiver_id = ? AND f.status = 'pending'`, [uid], (err, requests) => {
            ...
            db.all(`SELECT receiver_id FROM friends WHERE sender_id = ? AND status = 'pending'`, [uid], (err, pendingRows) => {
                ...
                db.all(`SELECT g.*, (SELECT COUNT(*) FROM group_members WHERE group_id = g.id) as memberCount FROM groups g JOIN group_members gm ON g.id = gm.group_id WHERE gm.user_id = ?`, [uid], (err, myGroups) => {
                    ...
                    // --- YENİ KISIM: HER TİM İÇİN ÜYELERİ BUL ---
...
// Chat ve Auth kısımları aynı (Kısaltıldı)
app.get('/chat', isAuthenticated, (req, res) => {
    ...
    db.all(`SELECT u.id, u.username, u.avatar FROM friends f JOIN users u ON (f.sender_id = u.id OR f.receiver_id = u.id) WHERE (f.sender_id = ? OR f.receiver_id = ?) AND f.status = 'accepted' AND u.id != ?`, [uid, uid, uid], (err, friends) => {
        ...
        db.all(`SELECT g.id, g.name FROM groups g JOIN group_members gm ON g.id = gm.group_id WHERE gm.user_id = ?`, [uid], (err, groups) => {
            ...
            // 3. Tim Üyelerini ve Okunmamış Mesajları Hesapla
            const tasks = [];

            // A) Arkadaşların okunmamış mesajları
            friends.forEach(f => {
            ...
            // B) Tim Üyelerini Çekme (YENİ KISIM)
            groups.forEach(g => {
                tasks.push(new Promise(resolve => {
...
app.post('/login', (req, res) => {
    db.get(`SELECT * FROM users WHERE email = ?`, [req.body.email], async (err, user) => {
        ...
        res.render('login', { error: 'Giriş yapılamadı.' });
    });
});
...
app.post('/register', async (req, res) => {
    try {
        ...
        db.run(`INSERT INTO users (username, email, password) VALUES (?, ?, ?)`, [req.body.username, req.body.email, hash], (err) => {
            if (err) return res.render('register', { error: 'Kullanımda.' });
            res.redirect('/login');
        });
    } catch (e) { res.render('register', { error: 'Hata.' }); }
});
...
io.on('connection', (socket) => {
    socket.on('join_room', (room) => { socket.join(room); db.all("SELECT * FROM messages WHERE room = ? ORDER BY created_at ASC LIMIT 50", [room], (err, msgs) => { socket.emit('load_history', msgs || []); }); });
    socket.on('send_message', (data) => {
        const timestamp = new Date().toISOString();
        db.run("INSERT INTO messages (room, sender_id, sender_name, sender_avatar, message, created_at) VALUES (?, ?, ?, ?, ?, ?)", [data.room, data.sender_id, data.author, data.avatar, data.message, timestamp], function() { io.to(data.room).emit('receive_message', { ...data, created_at: timestamp }); });
    });
    socket.on('clear_notifications', (userId) => { db.run("UPDATE messages SET is_read = 1 WHERE sender_id != ? AND room LIKE ?", [userId, `%${userId}%`]); });
});
